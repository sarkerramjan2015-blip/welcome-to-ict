...
const targetFile = path.join(__dirname, '../src/pages/MegaChallenge.tsx');
...
// Wait, I should just replace the whole function to be sure.
const getNextFallbackQuizStartOld = `const getNextFallbackQuizStart = () => {
...
const getNextFallbackQuizStartNew = `const getNextFallbackQuizStart = () => {
  const nowBdt = new Date(Date.now() + BDT_OFFSET_MS);
  const targetBdt = new Date(Date.UTC(
    nowBdt.getUTCFullYear(),
    nowBdt.getUTCMonth(),
...
if (content.includes(getNextFallbackQuizStartOld)) {
  content = content.replace(getNextFallbackQuizStartOld, getNextFallbackQuizStartNew);
}

// 2. Fix getFallbackChallenge
const getFallbackChallengeOld = `const getFallbackChallenge = (): UpcomingChallenge => ({
  id: 'monthly-quiz',
...
const getFallbackChallengeNew = `const getFallbackChallenge = (): UpcomingChallenge => {
...
if (content.includes(getFallbackChallengeOld)) {
  content = content.replace(getFallbackChallengeOld, getFallbackChallengeNew);
}
...
// 4. UI Replacement
const uiRegex = /<motion\.section[\s\S]*?<\/motion\.section>/;
const newUI = \`<motion.section
...
console.log('MegaChallenge successfully fixed and UI upgraded!');
